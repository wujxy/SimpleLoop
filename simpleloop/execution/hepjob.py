...
from ..world import SourceWorkspace, WorkspaceSpec, forwarded_payload_env
...
class HEPJobBackend(ExecutionBackend):
    ...
    def eval_baseline(self, *, baseline_sha: str) -> tuple[str, dict]:
        ...
        try:
            workspace = self.ctx.workspace.create(
                WorkspaceSpec(baseline_job_id, baseline_sha)
            )
            ...
            if worktree is not None and (result_dir / "_FINISHED").exists():
                ...
                try:
                    self.ctx.workspace.remove(SourceWorkspace(
                        baseline_job_id,
                        self.ctx.workspace.wt_root / f"r{baseline_job_id}",
                        baseline_sha,
                    ))
                except Exception:
                    pass
    ...
    def _prepare(self, plan: CandidatePlan, round_id: int) -> _Job:
        ...
        self._write_manifest(job, plan, round_id, workspace.path)
        return job
    ...
    def _retry_or_fail(self, job: _Job, note: str) -> None:
        ...
        if job.attempt < self.cfg["max_attempts"]:
            ...
            self.ctx.workspace.remove(SourceWorkspace(
                job.worktree_id,
                self.ctx.workspace.wt_root / f"r{job.worktree_id}",
                spec.parent_sha,
            ))
            workspace = self.ctx.workspace.create(
                WorkspaceSpec(job.worktree_id, spec.parent_sha)
            )
            spec.attempt = job.attempt
            spec.worktree_path = str(workspace.path)
            (job.result_dir / "manifest.json").write_text(
                json.dumps(spec.to_dict(), ensure_ascii=False, indent=2)
                + "\n", encoding="utf-8")
            self._submit(job)
    ...
    # Mirrors the candidate pipeline, but: (a) the workspace is a lane
        # workspace (create_lane/remove_lane); (b) the worker
    ...
    def _prepare_lane(self, lane_id: int, round_id: int, base_sha: str, *,
        ...
        result_dir.mkdir(parents=True, exist_ok=True)
        workspace = self.ctx.workspace.create_lane(lane_id, base_sha)
        ...
        spec = proposer_lane_worker.ProposerLaneSpec(
            ...
            run_dir=str(self.run_dir), workspace_path=str(workspace.path),
            result_dir=str(result_dir),
            prompt_dir=str(getattr(self.ctx, "prompt_dir", None) or ""),
    ...
    def _retry_or_fail_lane(self, job: _Job, note: str) -> None:
        ...
        if job.attempt < self.cfg["max_attempts"]:
            ...
            print(f"[{stamp()}] proposer lane {label}: {note}; retrying as "
                  f"attempt {job.attempt}", flush=True)
            self.ctx.workspace.remove_lane(SourceWorkspace(
                f"lane-{job.candidate_id}",
                self.ctx.workspace.lanes_root / f"lane-{job.candidate_id}" / "workspace",
                self._parent_sha,
            ))
            workspace = self.ctx.workspace.create_lane(
                job.candidate_id, self._parent_sha)
            spec = proposer_lane_worker.ProposerLaneSpec.from_dict(
                json.loads((job.result_dir / "manifest.json").read_text(
                    encoding="utf-8")))
            spec.attempt = job.attempt
            spec.workspace_path = str(workspace.path)
