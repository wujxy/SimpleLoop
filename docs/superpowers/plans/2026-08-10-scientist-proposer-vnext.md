...
**Architecture:** `ProposerOrchestrator` 只负责 lane、quota、并发和稳定聚合；每个 lane 创建一个独立 `ProposerAgent`，以 `ScientistSessionState` 持有累计 telemetry，并以 `ScientistContextState(runtime=WorkingState(), inquiry=InquiryState())` 持有当前 context 的认知状态。Scientist 连续性的 invariant 是同一个 session、artifact 与 evidence lineage，不是永不替换的 raw `messages` list；普通 phase boundary 可以压缩 transcript，但不得丢失已提交 artifact 与证据可见性。Fresh 阶段同时在 prompt、action guard、memory façade 和 Apptainer bind 四层锁住 search history；portfolio commit 后在同一 epistemic context 单调开启 history。只有 `fresh_reframe` 能清空 context-local epistemic state 并创建 clean context，其他 evidence conflict 按 direction/explanation/model 层级回退。
...
- vNext 的 strict fresh history lock 是为了建立干净实验条件的 policy，不宣称它是普遍科学方法；prior empirical evidence 提前可见性的变体留给后续 A/B，不阻塞本版。
- Phase commit 判断“当前 artifact 是否足以支持下一项 consequential research decision”，不判断知识是否 complete；调查优先回答会改变 representation、explanation 或 direction 的问题。
- 同一个 epistemic context 的 history visibility 只允许 `False → True`；允许不改变 `context_id` 的 transcript compaction，`fresh_reframe` 才会创建新 epistemic context，且第一版最多一次。
...
- 保留 G1–G9、每 lane 随机 5-of-9、lane quota、lane 并发、stable aggregation、Worker/Harness 外部接口和历史实验数据的只读审计能力。
- 这是非兼容架构重构：删除 `GeneratorAgent`、`HypothesisCard`、Sieve/Enrich、generator regeneration、partial proposal、旧 proposer config key 和旧 prompt snapshot schema；不得增加 façade、双路径、legacy-loadable prompt 或旧 runtime fallback。
...
- Modify: `simpleloop/memory/context.py` — 直接以 fresh world context 与 history-entry pack 取代旧 generation/startup view，不保留 compatibility façade。
- Modify: `simpleloop/memory/service.py` — 暴露 `build_history_entry_pack()`。
- Modify: `simpleloop/memory/models.py` — 向 `ResearchProposal` 增加 Scientist lineage metadata，同时迁移所有调用者。
- Create: `simpleloop/roles/generative.py` — G1–G9 常量和纯 prompt basis helper。
- Delete: `simpleloop/roles/generator.py`, `simpleloop/roles/hypothesis.py` — 不保留旧 Agent/card 边界。
...
- Modify: `simpleloop/config.py` — 只暴露 `scientist_steps`；移除 `gen_steps`/`cognitive_steps` proposer 预算路径。
...
- Modify: `simpleloop/prompts/__init__.py` — prompt 集合直接收敛为 proposer/executor/meta_optimizer。
- Rewrite: `simpleloop/prompts/proposer.md` — 唯一 active Scientist identity，不含 phase checklist 或 OMILREC leakage。
- Delete: `simpleloop/prompts/generator.md`
- Modify: `simpleloop/self_improvement/history.py`, `gate.py` — snapshot schema 直接迁移到三 prompt，不支持旧四 prompt snapshot 恢复。
...
- Delete/Rewrite: `tests/test_generator.py`, `tests/test_cognitive_element.py`; Modify: `tests/test_orchestrator.py` — 以 Scientist 行为测试取代旧架构测试，不保留兼容断言。
- Modify: `tests/test_config_execution.py`, `tests/test_parallel_candidates.py`, `tests/test_proposer_harness.py`, `tests/test_proposer_harness_runner.py`, `tests/test_proposer_cli.py`, `tests/test_prompt_templates.py`, `tests/test_prompt_self_improvement.py` — integration 与新架构边界测试。
...
For each checkpoint run `git diff --check`, inspect `git status --short`, and stage only the exact files named by that task. 本实施分支纳入已批准的 research、implementation 与本修订计划；不纳入 superseded plan。
...
- [ ] **Step 2: Audit the exact baseline task wording before any model call.**

Inspect `goal`, `hints`, gates, editable/frozen paths, and any startup context derived from them. Record whether they disclose benchmark-specific mechanisms or expected solutions. The current OMILREC config is expected to be neutral (`SPEED_MS` objective plus correctness gates); if and only if this audit finds contamination, add a separately recorded neutral-task suite. Do not mandate a second five-seed suite when the audited config is already neutral.

- [ ] **Step 3: Run five current proposer-only seeds.**
...
- [ ] **Step 4: Verify baseline artifacts mechanically.**
...
- Produces `InquiryPhase`, `Understanding`, `ModelClaim`, `WorkingModel`, `Explanation`, `LeveragePoint`, `ResearchHypothesis`, `HypothesisSelection`, `PhaseTransition`, `InquiryState`, `ScientistContextState`, and `ScientistSessionState`.
...
- `ScientistSessionState.start_fresh_context()` archives the complete parent `ScientistContextState`, increments `context_id`, creates a fresh `WorkingState` and `InquiryState`, and enforces `MAX_FRESH_REFRAMES = 1`. Only cumulative usage and the cumulative action log survive; no guard-affecting counts, fingerprints, observations, or evidence survive.
...
Fresh hypotheses may be conjectures: `model_basis`, `explanation_basis`, and `critical_unknown` are mandatory; `evidence_refs` is optional and, when present, must resolve to evidence seen in the current context. Direct evidence becomes mandatory only for model claims and the selected critical premise in DEEPEN.

`InquiryState` contains the artifacts plus `context_id`, `history_injected_at_step`, `narrow_decisions`, and `deep_evidence_refs`. `ScientistContextState` composes a fresh `WorkingState` with the current `InquiryState`. `ScientistSessionState` owns the current context, archived contexts, cumulative usage, cumulative action log, and reframe count. Context-local counters and evidence never live in cumulative telemetry.
...
- Replace the old generation/startup context builders with `build_fresh_inquiry_context()` and migrate every caller; no compatibility façade remains.
- Produce a thin `build_history_entry_pack(...) -> str` containing only a factual experiment index and memory-tool availability; abstentions, Findings, Explore, and Frontier interpretations are retrieved on demand.
...
def test_history_entry_pack_does_not_force_a_proposal():
    text = build_history_entry_pack(
        experiments=[], tool_cheatsheet="search_experiments(...)"
    )
...
Render a bounded factual index from experiment records: experiment ID, pass/fail status, objective metric, and changed scope. Append `MEMORY_TOOL_CHEATSHEET`. Do not render abstentions, Findings, Explore, Frontier, or any interpreted historical narrative at injection time; do not duplicate Ledger parsing. End the pack with:
...
Expected: PASS; all production callers use the two explicit new views and no old startup/generation façade remains.
...
```python
assert guard(commit_model_with_important_unknowns) is None
assert guard(commit_model_with_blocking_unknown) == "model_blocking_unknown"
...
Treat two materially different explanations as a target, not a minimum: one may commit only with a non-empty `explanation_sufficiency_justification` explaining which alternatives were considered and why none remains materially live. Test that commit rejects a stale model version, a hypothesis may omit direct evidence, and any portfolio below its computed breadth target requires a non-empty coverage justification; no positive hypothesis count is rejected solely by cardinality.
...
```json
{"action":"commit_understanding","problem":"...","target_outcome":"...","boundary":"...","current_account_of_the_whole":"...","key_unknowns":["..."]}
{"action":"continue_investigation","question":"...","decision_impact":"..."}
{"action":"commit_explanation_set","explanation_ids":["E1","E2"],"explanation_sufficiency_justification":null}
{"action":"commit_hypothesis_portfolio","hypothesis_ids":["H1","H2","H3","H4"],"coverage_rationale":"...","portfolio_sufficiency_justification":null,"unused_generative_ops":[]}
...
{"action":"commit_working_model","model_version":2,"model_check":{"explains_target":"...","counterfactual":{"change":"...","predicted_effect":"...","model_claim_refs":["M1"]},"important_unknowns":[{"question":"...","why_it_matters":"..."}],"blocking_unknown":null,"why_model_is_sufficient_for_next_stage":"..."}}
{"action":"submit_explanation","id":"E1","phenomenon":"...","account":"...","model_basis":["M1"],"expected_if_true":["..."],"evidence_needed":["..."]}
{"action":"emit_lever_map","levers":[{"id":"L1","target_mechanism":"...","why_leverage_exists":"...","model_basis":["M1"],"explanation_basis":["E1"]}]}
{"action":"submit_hypothesis","id":"H1","generative_op":"G2","model_basis":["M1"],"explanation_basis":["E1"],"mechanism":"...","intervention_family":"...","scope":"...","why_plausible":"...","critical_unknown":"..."}
...
Each `propose_working_model` assigns the next monotonic version and replaces the current provisional model. `commit_working_model` references exactly that current `model_version` plus `model_check`; it never resubmits a model. A stale or unknown version is rejected. `model_check.important_unknowns` may remain non-empty; only `model_check.blocking_unknown` must be `null`. The standard is sufficient for EXPLAIN, not complete knowledge.
...
Set `breadth_target = max(4, select_quota * 2)` distinct `ResearchHypothesis.signature()` values. Any non-empty portfolio below the target may commit only with a non-empty `portfolio_sufficiency_justification` whose coverage rationale names the attempted space and why further directions would be cosmetic or unsupported. Cardinality alone never rejects a non-empty portfolio. Treat 5 lenses × 2 ideas as exploration pressure, never an exact card quota. Validate hypothesis `evidence_refs` only when supplied; always require non-empty model/explanation lineage and `critical_unknown`.
...
- Fresh phases share one `ScientistSessionState` and one epistemic lineage; phase transitions are action-driven, not step-driven. Raw transcript identity is not an invariant, and phase-boundary compaction may replace `messages` while preserving committed artifacts and lineage.
- Portfolio commit switches tools, marks the current epistemic context history-visible, and injects the thin history pack exactly once into whichever transcript represents that context.
...
Assert the same `ScientistSessionState`, committed artifacts, and lineage survive even if a fake phase-boundary compactor replaces the message list; transitions are `understand→model→explain→explore→narrow`, and `history_injected_at_step` equals the portfolio commit step.
...
Do not create one Scientist session per phase. A bounded transcript compactor may run at phase boundaries without changing `context_id`; recreate `ResearchTools` only when visibility changes or a fresh reframe creates a new epistemic context.
...
- `propose_working_model` increments the provisional model version and replaces the current provisional model without transition。
- `commit_working_model` commits the referenced current version after guard success and transitions to EXPLAIN; it does not accept a repeated model payload。
...
Aggregate usage events into session-level `usage_by_phase[phase.value]`. Keep current-context observations, fingerprints, evidence sets, and guard counts under `session.context.runtime`; do not expose hidden reasoning or full model chain-of-thought.
...
- Create: `simpleloop/roles/generative.py`
- Modify: `simpleloop/roles/proposer.py`
- Delete: `simpleloop/roles/generator.py`, `simpleloop/roles/hypothesis.py`
- Create: `tests/test_generative.py`; Delete: `tests/test_generator.py`
- Modify: `tests/test_scientist_proposer.py`

**Interfaces:**

- `GENERATIVE_OPS`, `g_definition()`, and `replace_basis()` are pure helpers in `generative.py`.
- `GeneratorAgent`, `GenerationResult`, `HypothesisCard`, `run()`, `regenerate()`, and `_tool_loop()` are deleted with their modules after callers migrate.
...
- `ResearchHypothesis` is the only hypothesis type; old cards are not loadable runtime objects.

- [ ] **Step 1: Replace generator tests with pure generative-basis and Scientist EXPLORE.**
...
```bash
pytest -q tests/test_generative.py tests/test_scientist_proposer.py -k 'generative or lever or hypothesis'
```
...
After `rg` confirms callers are migrated, move only the pure G1–G9 basis into `generative.py`, then delete `generator.py` and `hypothesis.py`. Keep no compatibility alias or deserializer for either old runtime type.
...
```bash
pytest -q tests/test_generative.py tests/test_scientist_proposer.py
...
- `continue_explore`, `reopen_explain`, and `reopen_model` keep `history_visible=True` and the same epistemic context; transcript compaction remains allowed.
...
```python
session.archive_current_context()
session.context = ScientistContextState.fresh(
    context_id=old_context_id + 1,
)
...
Preserve only `session.cumulative_usage`, `session.cumulative_action_log`, and reframe count. Reset context-local action counts, protocol repairs used by guards, observations, evidence refs, tool fingerprints, source-read counts, and the entire inquiry artifact graph. Do not copy old assistant/user messages. When the new portfolio commits, inject history normally and continue NARROW.
...
- `ResearchProposal` requires Scientist lineage metadata in addition to instruction and research target; migrate every producer, serializer, and test atomically.
...
- [ ] **Step 1: Add failing required-lineage and trace-shape tests.**
...
- [ ] **Step 3: Replace the `ResearchProposal` schema atomically.**
...
Trace action entries contain action name, phase, context, step, and bounded observation summary/evidence refs. Record every provisional Working Model version so reviews can distinguish genuine evidence-driven revision from a single polished commit. Record steps to first committed Working Model, portfolio commit, and terminal outcome plus tool calls/tokens/wall time by phase. Do not store raw hidden chain-of-thought. Artifact objects use explicit `to_dict`/serializer functions so tuples become JSON lists and proposal targets retain existing serialization.
...
- `ProposerOrchestrator.run(..., scientist_steps: int = 364, random_seed: int | None = None)` replaces separate gen/cognitive budgets. `364` is only a hard safety ceiling inherited as a neutral default; it is neither a phase quota nor a target for depth. Early valid commitment is expected.
...
- [ ] **Step 3: Add the single Scientist budget contract tests.**

Required behavior:

```python
assert load_config(loop={}).scientist_steps == 364
assert load_config(loop={"scientist_steps": 40}).scientist_steps == 40
with pytest.raises(ConfigError):
    load_config(loop={"gen_steps": 20})
with pytest.raises(ConfigError):
    load_config(loop={"cognitive_steps": 12})
```

Reject `gen_steps` and `cognitive_steps` unconditionally; there is no mapping or precedence branch.
...
### Task 10: Replace the Proposer Identity and Remove the Generator Prompt Boundary
...
- `PROMPT_NAMES = ("proposer", "executor", "meta_optimizer")` is the only prompt schema.
- `load_semantic("generator")` is invalid and `generator.md` is deleted.
- Snapshot copy/compare/restore and prompt gate require exactly the three current prompts. Old four-prompt snapshots are outside the supported runtime architecture; no compatibility restore path is added.

- [ ] **Step 1: Add failing three-prompt schema tests.**

Assert `load_semantic("generator")` is rejected, new snapshots contain exactly the three prompts plus manifest, gate rejects an extra generator prompt, and restore accepts only the three-prompt schema.
...
Expected: FAIL on the current four-prompt and cognitive identity assumptions.
...
- [ ] **Step 5: Replace the prompt schema without a legacy branch.**
...
`load_semantic()`, history, restore, and gate all use `PROMPT_NAMES`. Delete generator-specific branches and the packaged prompt; do not add an old-snapshot adapter.
...
Expected: PASS, including crash recovery for the current three-prompt schema and explicit rejection of obsolete snapshots.
...
Expected: identify stale docs and tests that still need migration; no production role/orchestrator references are allowed at task completion.
...
- Proposal abstraction L0/L1/L2/L3 and the model-implied leverage level; judge their consistency, never rank higher abstraction as inherently better。
- Factual accuracy against source。
- Tool/token/wall-time cost。
- Steps to Working Model, portfolio, and outcome; model revision count and whether revisions follow new evidence。
...
The first vNext experiment keeps strict fresh history blindness. A later empirical-evidence-only variant is a separate A/B question, not implementation scope for this branch.
...
- [ ] `simpleloop.roles.generator`, `HypothesisCard`, old proposer budget keys, Sieve/Enrich, regeneration, and partial-proposal paths are removed rather than wrapped.
- [ ] One concurrent lane owns one isolated `ProposerAgent`, one `ScientistSessionState`, and one current epistemic context; transcript identity is not an invariant.
...
- [ ] G1–G9, 5-of-9 assignment, lane quotas, concurrency, stable aggregation, Worker/Harness, and historical experiment-data audit remain.
- [ ] Standalone harness records deterministic seed and complete structured trace.
- [ ] Prompt schema is exactly proposer/executor/meta_optimizer; no generator prompt or legacy proposer runtime is loadable.
