...
```json
{
  ...
  "model_check": {
    ...
    "remaining_decision_changing_unknown": null,
    "why_ready": "..."
  }
}
```
...
> 既然你自己判断这个未知可能改变问题模型或 broad search，就先调查它。
...
- `commit_explanation_set` 通常至少要求 2 个 distinct explanations；
- 允许只提交 1 个，但必须有：

```json
"single_explanation_justification":
...
```python
min_distinct_hypotheses = max(4, select_quota * 2)
```
...
- compact dashboard；
- Explore / Frontier summary；
- 如何按需 search experiments/findings。

Scientist 自己决定读哪些历史。

### 为什么仍然复用同一个 messages context
...
> Current mode: UNDERSTAND. Do not search for modifications yet. Investigate the problem broadly enough to understand what is being changed or explained, how the target outcome arises, and which unknowns could change your later model.

### MODEL

> Current mode: MODEL. Construct a working representation that can explain the target outcome and support counterfactual reasoning. A list of components or facts is not sufficient.

### EXPLAIN

> Current mode: EXPLAIN. Explain why the current gap or phenomenon occurs. Keep materially different explanations alive where evidence permits. Do not design the intervention yet.
...
```text
generator_steps + cognitive_steps
```
...
```python
scientist_steps = old_gen_steps + old_cognitive_steps
```
...
### Metric G — Proposal abstraction level
...
目标不是“所有 proposal 都必须 L2/L3”。

真正看的是：

> vNext 是否在没有 architecture hint 的情况下，自发提高 L2/L3 discovery rate。
